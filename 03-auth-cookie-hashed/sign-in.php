...
if(isset($_POST['sign-in']))
{
    ...
    // if a user is found, verify the entered password against the stored hashed password
        ...
        // if verification is successful, store the username and hashed password in cookie and redirect to homepage
        if(password_verify($password, $row['password'])) {
            $cookie_expiration = time() + (7 * 24 * 60 * 60); // 7 days
            setcookie('example3_username', $row['username'], $cookie_expiration);
            setcookie('example3_password', $row['password'], $cookie_expiration);
